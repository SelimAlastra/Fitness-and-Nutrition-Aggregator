  ...
  export const Likes = ({post}) => {
    ...
    if (post.likes.length > 0) {
      return post.likes.find((like) => like === (user?.result?._id))
        ? (
          <><FavoriteIcon fontSize="small" role="likeButton"/>&nbsp;{post.likes.length > 2 ? `You and ${post.likes.length - 1} others` : `${post.likes.length} like${post.likes.length > 1 ? 's' : ''}`}</>
        ) : (
          <><FavoriteBorderIcon fontSize="small" role="likeButton"/>&nbsp;{post.likes.length}</>
        );
    }
